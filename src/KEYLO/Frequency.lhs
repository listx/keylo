...
\ct{vowelBigrams} treats a bigram and its reverse as the same.

\begin{code}
vowelBigrams :: [(String, Double)]
vowelBigrams = foldl step [] vowelPairs
	where
	step acc (ab, ba) = (ab, abFreq + baFreq) : (ba, abFreq + baFreq) : acc
		where
		abFreq = lookupJustNote "incomplete bigrams" ab norvigBigramCountsRaw
		baFreq = lookupJustNote "incomplete bigrams" ba norvigBigramCountsRaw
	vowelPairs
		= map (\p -> (p, reverse p))
		$ pickUniq 2 "aeiouwy"

norvigBigramCountsRaw :: [(String, Double)]
norvigBigramCountsRaw = map (\(bgram, freq) -> (bgram, freq / freqSum)) raw
	where
	freqSum = sum $ map snd raw
	raw =
		[ ("th",100272945963)
		, ("he",86697336727)
		, ("in",68595215308)
